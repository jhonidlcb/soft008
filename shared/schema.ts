...
export const users = pgTable("users", {
  ...
  role: varchar("role", { length: 50 }).notNull().default("cliente"), // 'admin', 'cliente', 'socio'
...
export const projects = pgTable("projects", {
  ...
  status: varchar("status", { length: 50 }).notNull().default("pendiente"), // 'pendiente', 'negociando', 'en_progreso', 'completado', 'cancelado'
...
export const tickets = pgTable("tickets", {
  ...
  status: varchar("status", { length: 50 }).notNull().default("abierto"), // 'abierto', 'en_progreso', 'resuelto', 'cerrado'
  priority: varchar("priority", { length: 50 }).notNull().default("media"), // 'baja', 'media', 'alta', 'urgente'
  userId: integer("user_id").references(() => users.id).notNull(),
...
export const paymentStages = pgTable("payment_stages", {
  ...
  status: text("status").notNull().default("pendiente"), // pendiente, disponible, pagado
...
export const notifications = pgTable("notifications", {
  ...
  type: varchar("type", { length: 50 }).notNull().default("info"), // 'info', 'exito', 'advertencia', 'error'
...
export const projectTimeline = pgTable("project_timeline", {
  ...
  title: varchar("title", { length: 255 }).notNull(),
  description: text("description"),
  status: varchar("status", { length: 50 }).default("pendiente"), // 'pendiente', 'en_progreso', 'completado'
...
export const budgetNegotiations = pgTable("budget_negotiations", {
  ...
  status: varchar("status", { length: 50 }).notNull().default("pendiente"), // 'pendiente', 'aceptado', 'rechazado', 'contrapuesta'
  createdAt: timestamp("created_at").defaultNow(),
...
export const referrals = pgTable("referrals", {
  ...
  status: varchar("status", { length: 50 }).notNull().default("pendiente"), // 'pendiente', 'pagado'
  createdAt: timestamp("created_at").defaultNow(),
});
...
export const invoices = pgTable("invoices", {
  ...
  amount: decimal("amount", { precision: 12, scale: 2 }).notNull(),
  status: varchar("status", { length: 50 }).notNull().default("pendiente"), // 'pendiente', 'pagado', 'vencido', 'cancelado'
...
export const transactions = pgTable("transactions", {
  ...
  status: varchar("status", { length: 50 }).notNull().default("pendiente"), // 'pendiente', 'completado', 'fallido', 'reembolsado'
  transactionId: varchar("transaction_id", { length: 255 }),
  paymentData: jsonb("payment_data"),
  createdAt: timestamp("created_at").defaultNow(),
  completedAt: timestamp("completed_at"),
