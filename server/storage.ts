...
export class DatabaseStorage implements IStorage {
  ...
  async getPartnerEarningsData(partnerId: number): Promise<any> {
    try {
      ...
      // Ganancias mensuales (mes actual)
      ...
      // Referidos activos (con proyectos)
    ...
    try {
      ...
      stats.forEach((stat) => {
        ...
        if (status !== 'totalRevenue' && status !== 'potentialRevenue') {
          result[status as keyof typeof result] = stat.count?.toString() || "0";
          
          // Map to English keys for compatibility
  ...
  async updateProjectTimeline(timelineId: number, updates: any): Promise<ProjectTimeline> {
    try {
      ...
      if (updates.status === 'completed' || updates.status === 'completado') {
        ...
        // 1. Forzar actualización de progreso
        await this.updateProjectProgressBasedOnTimeline(timeline.projectId);
        
        // 2. Pequeña pausa para asegurar persistencia en DB antes de leer
        await new Promise(resolve => setTimeout(resolve, 500));

        try {
          // 3. Volver a obtener el proyecto con el progreso ya actualizado
          const [project] = await db.select().from(projects).where(eq(projects.id, timeline.projectId));
          
          if (project) {
            const currentProg = Number(project.progress);
            console.log(`📊 [DEBUG PAYMENTS] Proyecto ${project.id} - Progreso calculado: ${currentProg}%`);
            
            const allStages = await db.select().from(paymentStages).where(eq(paymentStages.projectId, project.id));
            
            // Filtramos etapas que no estén pagadas ni ya disponibles
            const pendingStages = allStages.filter(s => {
              const status = (s.status || '').toLowerCase();
              return status !== 'pagado' && status !== 'available' && status !== 'disponible';
            });
            
            console.log(`📋 [DEBUG PAYMENTS] Revisando ${pendingStages.length} etapas para activación potencial`);
            
            for (const stage of pendingStages) {
              const reqProg = Number(stage.requiredProgress);
              
              if (currentProg >= reqProg) {
                console.log(`💰 [DEBUG PAYMENTS] ¡REQUISITO CUMPLIDO! Etapa "${stage.stageName}" (Req: ${reqProg}%, Actual: ${currentProg}%)`);
                
                // Actualizar a 'available' (usado por el sistema de emails)
                ...
                const { notifyPaymentStageAvailable } = await import("./notifications");
                ...
                console.log(`📧 [DEBUG PAYMENTS] Notificación enviada para: ${stage.stageName}`);
              }
            }
          }
        } catch (paymentErr) {
          console.error("❌ Error en cadena de activación de pagos:", paymentErr);
        }
      }
  ...
  async updateProjectProgressBasedOnTimeline(projectId: number): Promise<void> {
    try {
      ...
      // PESO PERSONALIZADO SEGÚN TÍTULO
      let progressPercentage = Math.round((completedCount / totalCount) * 100);

      // Normalizar títulos para búsqueda (minúsculas y sin acentos básicos)
      ...
      if (hasEntrega) {
        ...
        progressPercentage = 90; // Forzamos a 90% para Fase 2
      } else if (hasFase1) {
        progressPercentage = 50; // Forzamos a 50% para Fase 1
      }
      
      // LOG CRÍTICO PARA DEPURACIÓN
      console.log(`📊 [DEBUG PROGRESS] Proyecto ${projectId}:`);
      console.log(`   - Ítems en DB: ${allTimeline.map(it => `${it.title}(${it.status})`).join(', ')}`);
      console.log(`   - Completados detectados: ${completedCount}/${totalCount}`);
      console.log(`   - Títulos clave: Fase1=${hasFase1}, Fase2=${hasFase2}, Entrega=${hasEntrega}`);
      console.log(`   - Porcentaje Resultante: ${progressPercentage}%`);

      // Actualizar el progreso del proyecto (usando db.update directamente para asegurar persistencia inmediata)
