...
interface Project {
  ...
  description: string;
  price: string;
  status: 'pendiente' | 'en_progreso' | 'completado' | 'cancelado' | 'negociando';
  progress: number;
...
export default function ProjectManagement() {
  ...
  const createTimelineMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setNewTimelineItem({ title: "", description: "", estimatedDate: "", status: "pendiente" });
      toast({
  ...
  const getStatusBadge = (status: string) => {
    const variants = {
      pendiente: { variant: "outline" as const, icon: Clock, color: "text-yellow-500" },
      en_progreso: { variant: "secondary" as const, icon: Settings, color: "text-blue-500" },
      completado: { variant: "default" as const, icon: CheckCircle, color: "text-green-500" },
      cancelado: { variant: "destructive" as const, icon: XCircle, color: "text-red-500" },
      negociando: { variant: "outline" as const, icon: DollarSign, color: "text-orange-500"},
    };
    ...
    return (
      <Badge variant={config.variant} className="flex items-center gap-1">
        ...
        {status === 'pendiente' ? 'Pendiente - Solicitud Original' :
         status === 'en_progreso' ? 'En Desarrollo' :
  ...
  const handleUpdateTimelineStatus = (timelineId: number, status: string) => {
    const updates = { 
      status,
      ...(status === 'completado' && { completedAt: new Date().toISOString() })
    };
            ...
            <CardContent className="p-6">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold">{projectStats?.pendiente || projectStats?.pending || 0}</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-6">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold">{projectStats?.en_progreso || projectStats?.inProgress || 0}</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-6">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold">{projectStats?.completado || projectStats?.completed || 0}</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-6">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold">{projectStats?.cancelado || projectStats?.cancelled || 0}</p>
                </div>
              </div>
            </CardContent>
          </Card>
              ...
              <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto">
                ...
                <Select value={statusFilter} onValueChange={setStatusFilter}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="en_progreso">En Desarrollo</SelectItem>
                    <SelectItem value="completado">Completados</SelectItem>
                    <SelectItem value="cancelado">Cancelados</SelectItem>
                    <SelectItem value="negociando">Negociando Presupuesto</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
                          ...
                          <div className="flex items-center gap-2">
                            <Badge 
                              variant={
                                ...
                                item.status === 'en_progreso' ? 'secondary' :
                                item.status === 'negociando' ? 'outline' : 'outline'
                              }
                              className={item.status === 'negociando' ? "text-orange-500 border-orange-500" : ""}
                            >
                              {item.status === 'completado' ? 'Completado' :
                               item.status === 'en_progreso' ? 'En Progreso' :
                               item.status === 'negociando' ? 'Negociando' :
                               'Pendiente'}
                            </Badge>
                            <select
                              value={item.status}
