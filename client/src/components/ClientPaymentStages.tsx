...
export default function ClientPaymentStages({ 
  ...
  const getNextPaymentStage = () => {
    ...
    const nextPayment = stages?.find((stage: PaymentStage) => 
      stage.status === 'pending' || stage.status === 'pendiente' || stage.status === 'unavailable' || stage.status === 'no_disponible'
    );
    return nextPayment || stages?.[0];
  };
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2">
                  <h3 className={`font-bold text-xl ${
                    ...
                    {(nextPayment.status === 'pending' || nextPayment.status === 'pendiente' || nextPayment.status === 'unavailable' || nextPayment.status === 'no_disponible') && 'ℹ️ Etapa Pendiente'}
                  </h3>
